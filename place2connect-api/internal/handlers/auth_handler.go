...
func (h *Handler) SignUpUser(c *fiber.Ctx) error {
	...
	var payload *handlersRequests.SignUpInput
	...
	err = h.userRepository.RegisterUser(newUser)
	if err != nil && (strings.Contains(err.Error(), "duplicate key value violates unique constraint \"idx_users_email\"") || strings.Contains(err.Error(), "duplicate key value violates unique constraint \"idx_email\"")) {
		return c.Status(fiber.StatusConflict).JSON(fiber.Map{"status": "fail", "message": "User with that email already exists"})
	} else if err != nil {
		return c.Status(fiber.StatusBadGateway).JSON(utils.BadGateway())
	}

	userResp := handlersResponses.FilterUserRecord(&newUser)

	return c.Status(fiber.StatusCreated).JSON(fiber.Map{"status": "success", "data": fiber.Map{"user": userResp}})
}
...
func (h *Handler) SignInUser(c *fiber.Ctx) error {
	var payload *handlersRequests.SignInInput
	...
	return c.Status(fiber.StatusOK).JSON(fiber.Map{"status": "success", "access_token": accessTokenDetails.Token, "user": handlersResponses.FilterUserRecord(user), "roles": userRoles})
}
